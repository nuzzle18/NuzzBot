...
handler.before = async function (m) {
    ...
    if (DATABASE.data.users[m.sender].suit < 0) DATABASE.data.users[m.sender].suit = 0
...
wa.me/${this.user.jid.split`@`[0]}`.trim())
            ...
            room.waktu_milih = setTimeout(async () => {
                if (!room.pilih && !room.pilih2) this.reply(m.chat, `kedua pemain tidak niat main, suit berakhir`)
                    ...
                    DATABASE.data.users[win == room.p ? room.p : room.p2].exp += room.poin
                    DATABASE.data.users[win == room.p ? room.p2 : room.p].exp -= room.poin_lose
                }
                delete this.suit[room.id]
                return !0
            }, room.timeout)
