	...
	if (!m.fromMe && m.text.match(/(beb|Beb|yank|yang|sayang|syg|syang|beby|baby|Yang|Sayang|Syg|Beby|by|By|Baby|😍|😘|🥰|😗|😙|😚|😻)/gi)) {
		...
		var warning = global.DATABASE.data.users[m.sender].warning
			if(warning > 9) {
				...
				conn.reply(m.chat, `*⺄1�7 ANTIBUCIN DETECTOR ⺄1�7*\n\n*Kamu mendapat peringatan : [ ${warning} / 10 ]*\n\n*Jangan Bucin karna admin iri, kalo mau bucin chat aja, kalo ngebucin sebanyak 10x kamu akan dikeluarkan dari grup secara otomatis.*\n\nGunakan command *.maaf* untuk menghapus 1 warning\n\n ▌│█║▌║▌║║▌║▌║█│▌▌│█║`, m)
			}
		}
	}
	
	if (!m.fromMe && m.text.match(/(6287743038495)/gi)) {
stiker = await sticker(false, "https://telegra.ph/file/dfee29786ff4c524443c2.png", "NUZZ-BOT", "Ngetag lagi saya benet")
	...
	if(enable.nobomchat && !m.fromMe && m.isGroup && isBotAdmin) {
		...
		var warning = global.DATABASE.data.users[m.sender].warning
			if(warning > 9) {
				...
				conn.reply(m.chat, `*⺄1�7 BOMCHAT DETECTOR ⺄1�7*\n\n*Kamu mendapat peringatan : [ ${warning} / 10 ]*\n\n*Jangan Bomchat sebanyak 10x atau kamu akan dikeluarkan dari grup secara otomatis.*\n\n▌│█║▌║▌║║▌║▌║█│▌▌│█║`, m)
			}
		}
	}
	
	if (!m.fromMe && m.text.match(/(tq|thanks|tengs|tengkyu|arigato|mksih)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/makasih.opus', 'tts.opus', null, m, true)
	}
	
if (!m.fromMe && m.text.match(/(terimakasih|makasih)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/masama.opus', 'tts.opus', null, m, true)
	}
	
if (!m.fromMe && m.text.match(/(halo|halo bot|cara pake bot)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/hai.opus', 'tts.opus', null, m, true)
	}

if (!m.fromMe && m.text.match(/(aku sedih|jd sedih|jadi sedih|sadboy|sadgirl)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/sad.opus', 'tts.opus', null, m, true)
	}

if (!m.fromMe && m.text.match(/(cium dong|kiss dong|kiss me|ciuman dong|mwah)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/mwah.opus', 'tts.opus', null, m, true)
	}

if (!m.fromMe && m.text.match(/(desah dong)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
...
if (!m.fromMe && m.text.match(/(asalamualaikum|assalamu'alaikum|assalamualaikum)/gi)) {
	...
	if (!m.fromMe && m.text.match(/(asadebangsat|Dakjal|anak setan|ngntd|ngentot|jancuk|kuntul|babi|kampang|kenthu|tempik|kimak|patek|kondom|jancok|jembut|bokep|xnxx|xxx|xvideos|xvid|jilboob|seksi|Anjing|Babi|Kunyuk|Bajingan|Bangsat|Kampret|Kontol|Memek|Ngentot|Pentil|Perek|Pepek|Pecun|Bencong|Banci|Maho|Sinting|Lonte|Hencet|Taptei|Kampang|Keparat|Bejad|Gembel|Brengsek|Taek|Fuck|Tetek|Ngulum|ajg|aswJembut|Totong|Kolop|Pukimak|Bodat|Heang|Jancuk|Burit|Titit|Nenen|Bejat|Silit|komtol|Komtol|Sempak|Fucking|Asshole|Bitch|Penis|Vagina|Klitoris|Kelentit|Borjong|Dancuk|Pantek|kondom|Teho|Bejat|Pantat|Bagudung|Babami|Kanciang|Bungul|Idiot|Kimak|Henceut|Kacuk|pukimak|Pussy|ngewe|Dick|Damn|Assu|tempek|celeng|shit|jingan|ngentot|anjing|ngewe|kontol|ngentod|colmek|alat vital|bangkinang|tits|tetek|coli|ngocok peli|ANJING!!!|kntl|ngtd|anying|amjin|sikontol|bang bros|ngocok|toket|A n j i n g|Tahi|biadap|bbii|biadab|Tomlol|dongo|dungu|anjk|bcot|BURUNG KECIL JAN SOK KERAS:V|nude|p3n1s|p3nis)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/', 'tts.opus', null, m, true)
	}
	
	if (!m.fromMe && m.text.match(/(yank|ayank)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/sayang.opus', 'tts.opus', null, m, true)
	}
	
	if (!m.fromMe && m.text.match(/(diem dek|halo dek|bertumbuk|gelut|apasih dek|kontol amat dek|gelud yuk|gelud yok)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/jagoan.opus', 'tts.opus', null, m, true)
	}
	
	if (!m.fromMe && m.text.match(/(tapi boong|tp boong|tapi lombo|tapi bohong)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/tapi.opus', 'tts.opus', null, m, true)
	}
	
	if (!m.fromMe && m.text.match(/(iri bilang bos|iri bilang|iri bilang babi|iri blng bos)/gi)) {
	    conn.updatePresence(m.chat, Presence.recording)
	    conn.sendFile(m.chat, 'src/biasalah.opus', 'tts.opus', null, m, true)
	}
